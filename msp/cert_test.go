...
func TestSanitizeCertInvalidInput(t *testing.T) {
	...
	_, err = sanitizeSM2SignedCert(&sm2.Certificate{}, nil)
	assert.Error(t, err)
	...
	sigma, err := gm.MarshalSM2Signature(big.NewInt(1), sm2.P256Sm2().Params().N)
	assert.NoError(t, err)
...
func TestSanitizeCert(t *testing.T) {
	...
	for {
		...
		_, s, err := gm.UnmarshalSM2Signature(cert.Signature)
		assert.NoError(t, err)
	...
	_, s, err := gm.UnmarshalSM2Signature(sanitizedCert.Signature)
	assert.NoError(t, err)
