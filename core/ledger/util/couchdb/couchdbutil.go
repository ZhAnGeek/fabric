...
func ConstructMetadataDBName(dbName string) string {
	if len(dbName) > maxLength {
		...
		// (i.e., chainNameAllowedLength) of chainName> + (SHA256 or SM3 hash of actual chainName)
		dbName = dbName + "(" + hex.EncodeToString(util.ComputeHash([]byte(untruncatedDBName))) + ")"
		// 50 chars for dbName + 1 char for ( + 64 chars for (SHA256 or SM3) + 1 char for ) = 116 chars
	}
	return dbName + "_"
}
...
func ConstructNamespaceDBName(chainName, namespace string) string {
	...
	if len(namespaceDBName) > maxLength {
		...
		hashOfNamespaceDBName := hex.EncodeToString(util.ComputeHash([]byte(chainName + "_" + namespace)))
